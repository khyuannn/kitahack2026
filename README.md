...
**Price constraints and early settlement trigger**: Each party operates within hidden price bounds — the plaintiff sets a floor price (minimum acceptable settlement) and the defendant's maximum offer defaults to 50% of the claim amount, or a custom ceiling entered during PvP onboarding. Agents are instructed never to propose outside their respective bound. Critically, if the defendant's counter-offer meets or exceeds the plaintiff's floor price at any point during the negotiation, the system immediately surfaces an accept/reject decision to the plaintiff rather than waiting for round 4 — allowing the dispute to settle early. Conversely, if the plaintiff's demand falls within the defendant's ceiling, an equivalent early decision is triggered for the defendant. This mechanism ensures that once a zone of agreement is reached, the negotiation concludes efficiently without burning unnecessary rounds.
